...
pushl $2 # Second argument.
pushl $10 # First argument.
call power
addl $8, %esp

popl %ebx # Put second answer in ebx.
addl %eax, %ebx # result (in ebx) = second answer (in ebx) + third answer (in eax)

popl %eax # Put first answer in eax.
addl %eax, %ebx # result (in ebx) = previous answer (in ebx) + first answer (in eax)
